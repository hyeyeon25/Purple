  ...
  for query_obj in queries:
    ...
    try:
      ...
      for el in valid_elements:
        try:
          text = el.text.strip()
          # 화면 글자에 내 검색어가 있거나, 내 검색어에 화면 글자가 있거나
      ...
      if not clicked and search_type == "strict":
        first_el = valid_elements[0]
        print(f" 이름 불일치. 하지만 주소 검색이므로 첫 번째 가게('{first_el.text}')를 강제 클릭합니다.")
        try:
          first_el.click()
          time.sleep(3)
          clicked = True
...
def get_naver_reviews(place_name, address):
  ...
  try:
    ...
    for candidate in candidates:
      ...
      if found:
        print(f"사용된 클래스명: {candidate} ({len(found)}개)")
...
if __name__ == "__main__":
  print("파이썬 서버 시작 (http://localhost:8000)")
